import icon1 from '../../../public/image/servicos/1.png';
import icon2 from '../../../public/image/servicos/2.png';
import icon3 from '../../../public/image/servicos/3.png';
import icon4 from '../../../public/image/servicos/4.png';
import icon5 from '../../../public/image/servicos/5.png';
import icon6 from '../../../public/image/servicos/6.png';

export const servicosData = [
    {
        id: 1,
        icon: icon1,
        title: 'Criação de aplicativos',
        description: `Desenvolvimento de Aplicativos para Celulares Android e iOS (iPhone).`,
    },
    {
        id: 2,
        icon: icon2,
        title: 'Criação de sites',
        description: `Desenvolvimento de Sites, landpage e sistemas`,
    },
    {
        id: 3,
        icon: icon3,
        title: 'Loja virtual/ Ecommerce',
        description: `Criação de Loja Online, venda de produtos ou receba pagamentos através da Internet.`,
    },
    {
        id: 4,
        icon: icon4,
        title: 'Desenvolvedor back-end',
        description: `Especialista em Node, Express, Nest, MongoDB, Postgrees, Mysql...`,
    },
    {
        id: 5,
        icon: icon5,
        title: 'Desenvolvedor front-end',
        description: `Especialista em HTML5, JavaScript, CSS, Next, React.js...`,
    },
    {
        id: 6,
        icon: icon6,
        title: 'Sistema integrado',
        description: `Sistemas multiplataformas, onde dispositivos se comunicam e trabalham em sincronia.
        `,
    },
]